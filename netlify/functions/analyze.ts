...
const analyzeArticle = async (content: string, isUrl: boolean, mimeType?: string): Promise<AnalysisResult> => {
  try {
...
export default async (request: Request) => {
  ...
  try {
    const { content, isUrl, mimeType } = await request.json();

    if (typeof content !== 'string' || !content.trim()) {
      return Response.json({ error: 'Content is required.' }, { status: 400 });
    }

    const result = await analyzeArticle(content, Boolean(isUrl), mimeType);
